#include "debugger-settings.moc"
DebuggerSettingsWindow *debuggerSettingsWindow;
...
void DebuggerSettingsWindow::initializeUi() {
  lstring part;
...
void DebuggerSettingsWindow::videoDriverChange(int index) {
  if(index >= 0) config().system.video = videoDriver->itemText(index).toUtf8().data();
}

void DebuggerSettingsWindow::audioDriverChange(int index) {
  if(index >= 0) config().system.audio = audioDriver->itemText(index).toUtf8().data();
}

void DebuggerSettingsWindow::inputDriverChange(int index) {
  if(index >= 0) config().system.input = inputDriver->itemText(index).toUtf8().data();
}

void DebuggerSettingsWindow::setRegionAuto() { SNES::config.region = SNES::System::Region::Autodetect; }
void DebuggerSettingsWindow::setRegionNTSC() { SNES::config.region = SNES::System::Region::NTSC; }
void DebuggerSettingsWindow::setRegionPAL()  { SNES::config.region = SNES::System::Region::PAL; }

void DebuggerSettingsWindow::setPortSatellaview() { SNES::config.expansion_port = SNES::System::ExpansionPortDevice::BSX; }
void DebuggerSettingsWindow::setPortNone()        { SNES::config.expansion_port = SNES::System::ExpansionPortDevice::None; }

void DebuggerSettingsWindow::pauseWithoutFocus()       { config().input.focusPolicy = Configuration::Input::FocusPolicyPauseEmulation; }
void DebuggerSettingsWindow::ignoreInputWithoutFocus() { config().input.focusPolicy = Configuration::Input::FocusPolicyIgnoreInput; }
void DebuggerSettingsWindow::allowInputWithoutFocus()  { config().input.focusPolicy = Configuration::Input::FocusPolicyAllowInput; }

void DebuggerSettingsWindow::toggleRewindEnable() {
  config().system.rewindEnabled = rewindEnable->isChecked();
  state.resetHistory();
}

void DebuggerSettingsWindow::toggleUseCommonDialogs() {
